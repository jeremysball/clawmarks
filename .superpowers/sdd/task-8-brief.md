...
Context: this is the last task of Phase 1 of the preference-classifier plan. It's independent
of Tasks 1-7 (no shared files) — it adds a new module that later phases (preference model
...
- Produces: `embed_cache.EMBEDDINGS_FILE: Path`, `embed_cache.embed_paths(paths, model, batch_size=16) -> np.ndarray`,
  ...
  Later tasks (outside this plan's phase) consume `load_cache`/`sync`/`EMBEDDINGS_FILE`.
...
- This is the implementation exactly as written in the plan — transcribe it, don't redesign it.
