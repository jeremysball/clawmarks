...
def healthcheck():
    ...
    override, else tailscale_ip()) rather than hardcoding 127.0.0.1: unlike a server that binds
    0.0.0.0, main() binds to the tailscale interface specifically (or 0.0.0.0 only as a fallback
    when tailscale isn't up), so 127.0.0.1 would refuse the connection in the common case."""
    host = os.environ.get("CLAWMARKS_HOST") or tailscale_ip()
    url = f"http://{host}:{DEFAULT_PORT}/healthz"
    try:
        # proxies={} disables urllib's default HTTP_PROXY/HTTPS_PROXY env honoring: a proxy
        # meant for outbound egress must never intercept this loopback-adjacent request.
