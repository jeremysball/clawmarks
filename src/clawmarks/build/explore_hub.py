...
def render_html(active_expedition=None, active_leg=None, running=None, data=None, context=None):
    ...
    tool_index = "".join(f'<section><h2>{_esc(group)}</h2><div class="tool-index">' + "".join(f'<a href="{_esc(context_url(path, context))}"><span class="name">{_esc(descriptions[path][0])}</span><span class="desc">{html.escape(descriptions[path][1], quote=False)}</span></a>' for path, _ in group_tools) + "</div></section>" for group, group_tools in groups)
